...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear all
global beta mu A delta alpha s b N prob kk gridk probst v tiny
...
[prob,logs,invdist]=markovappr(rho,sigma,3,N);
...
while (testk>0.0001)
    ...
    k1= aiyagari(r0); %calculating asset demand   
...
figure(2)
plot(gridk,[dist(1,:)' dist(7,:)'],  '-.','linewidth', WD)
ylabel('distribution')
...
figure(3)
plot(gridk,[cons(1,:)' cons(4,:)' cons(7,:)'], '-.','linewidth', WD)
ylabel('consumption')
xlabel('asset holdings')
set(gca,'XGrid','off','YGrid','on','Fontsize',FS)
set(gca,'TickLabelInterpreter','LaTex')
...
figure(4)
plot(gridk,[v(1,:)' v(4,:)' v(7,:)'], '-.','linewidth', WD)
