...
incgridk = 0.2;                    % size of capital grid increments
...
while test > 0.001     
  for j=1:N
   for i=1:ngridk
      ...
      c_max       = (wealth(i,j)-gridk(1));          % highest possible consumption: consume acc cash on hand and max out on borrowiing going forward

      if test<0.5d0 
      cc=c0(i,j);
      else
      cc =  (c_min+c_max)/2 ; % initial guess   
      end
      
      while convergence>0.0001
      %% First order condition -- euler
      f       =  cc^(-mu)- beta*(1+r)*(interp1(gridk,c0,wealth(i,j)-cc).^(-mu))*prob(j,:)' ;
      %% Derivative of euler
      df      =  -mu*cc^(-mu-1)- (mu)*beta*(1+r)*(interp1(gridk,c0,wealth(i,j)-cc).^(-mu-1))*prob(j,:)' ;
      %% Find a new guess
      cc_new  =  cc-f./df;
      %% Check Convergence
      convergence=abs(cc_new-cc);
      %% Update guess
      cc=cc_new;
      end
      c1(i,j)=cc;
