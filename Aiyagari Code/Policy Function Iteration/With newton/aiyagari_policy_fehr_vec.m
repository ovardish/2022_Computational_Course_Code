...
c1=c0;
ready=zeros(ngridk,1);
while test > 0.001     
  for j=1:N
      j;
       % agent is borrowing constraine3d
        ready(((wealth(:,j)-gridk(1)).^(-mu) >= beta*(1+r)*prob(j,:)*(c0(1,:)').^(-mu)),1)=1;
        ii=find(ready==1);
        c1(ii,j)=wealth(ii, j) -gridk(1);
        
            
       % agent is savings constrained
           ready((wealth(:,j)-gridk(ngridk)).^(-mu) <= beta*(1+r)*prob(j,:)*(c0(ngridk,:)').^(-mu) & wealth(:,j)>gridk(ngridk) & ready==0,1)=2;
           iii=find(ready==2);
           c1(iii,j)=wealth(iii,j)-gridk(ngridk);
      
           
      iiii=find(ready==0);
      % asset decision is interior, using bisection
      erreuler=1;
      c_min       = max(0,(wealth(iiii,j)-gridk(end))); % lowest possible consumption: zero or whatever left if choosing max saving
      c_max       = (wealth(iiii,j)-gridk(1));          % highest possible consumption: consume all cash on hand and max out on borrowiing going forward
      ...
      while erreuler>0.0001
       % f=@(ll)  ll^(-mu)- beta*(1+r)*(interp1q(gridk,c0,wealth(i,j)-ll).^(-mu))*prob(j,:)' ;
       % df =@(ll)  -mu*ll^(-mu-1)- (mu)*beta*(1+r)*(interp1q(gridk,c0,wealth(i,j)-ll).^(-mu-1))*prob(j,:)' ;
      f  =  ll.^(-mu)- beta*(1+r)*(interp1(gridk,c0,wealth(iiii,j)-ll).^(-mu))*prob(j,:)' ;
      df =  -mu*ll.^(-mu-1)- (mu)*beta*(1+r)*(interp1(gridk,c0,wealth(iiii,j)-ll).^(-mu-1))*prob(j,:)' ;
      ll_new=ll-f./df;
      erreuler=max(abs(ll_new-ll));
      ll=ll_new;
      end
