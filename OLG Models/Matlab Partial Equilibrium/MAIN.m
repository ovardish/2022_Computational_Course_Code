...
%% Should i do it here, or even down?   
   for ij=JJ-1:-1:1
       ...
       for ia=1:NA+1
           % Check if individuals have zero resources
           ...
           for ip=1:ip_max
               ...
               %% we can easily check whether the  borrowing constraint is vioalted x(1)<0, and manually rewrite it;                
    ...
    for ia=1:NA+1
        for ip=1:NP
            for is=1:NS
                 ...
                 % Check whether the bounds are recovered.
                 %ial=min(ial,NA);
                 %iar=min(iar, NA);
